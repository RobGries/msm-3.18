...
#endif
/*
static ssize_t himax_reg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	ssize_t rc = -EINVAL;
	int reg;
	u8 val;
	...
	printk("%s: temp = %lu, reg 0x%02x = 0x%02x\n", __func__, temp, reg, val);
	...
	smb1351_write_reg(chip_smb1351, reg, val);

	return len;
}*/

struct i2c_client *himax_client;
static ssize_t  himax_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	//ssize_t ret;
	//u8 val;
	uint8_t val;
	...
	//for(reg = 0; reg < 72; reg++) {
	//	ret = smb1351_read_reg(chip_smb1351, reg, &val);

	//	if (ret) {
	//		printk("%s: failed to read smb1351 reg\n", __func__);
	//		return ret;
	//	}
	//
	//	n += sprintf(buf + n, "%x:%x\n", reg, val);
	...
	for(reg = 0; reg < 100; reg++) {
		if (i2c_himax_read(himax_client, reg, &val, 1, 10) < 0)
			printk("%s: failed to read reg\n", __func__);
		n += sprintf(buf + n, "%x:%x\n", reg, val);
	}
...
static DEVICE_ATTR(reg, 0664, himax_reg_show, NULL);
...
static struct attribute_group himax_attr_group = {
...
#endif
	err = sysfs_create_group(&client->dev.kobj, &himax_attr_group);
	if (err)
		printk("%s: sysfs @test\n", __func__);
