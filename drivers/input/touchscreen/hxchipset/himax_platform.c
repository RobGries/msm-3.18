...
static int himax_regulator_configure(struct i2c_client *client,struct himax_i2c_platform_data *pdata)
{
	...
	if (IS_ERR(pdata->vcc_dig)) {
		printk("%s: Failed to get regulator vdd @test\n", __func__);
	...
	if (IS_ERR(pdata->vcc_ana)) {
		printk("%s: Failed to get regulator avdd @test\n", __func__);
...
{
	int error;
	struct pinctrl *himax_pinctrl = devm_pinctrl_get(&client->dev);
	struct pinctrl_state *state;

	if (IS_ERR_OR_NULL(himax_pinctrl))
		printk("%s: failed to get inctrl: %ld\n", __func__, PTR_ERR(himax_pinctrl));
	...
	state = pinctrl_lookup_state(himax_pinctrl, "himax_irq");
	if (IS_ERR_OR_NULL(state))
		printk("%s: failed to get pinctrl state\n", __func__);
	error = pinctrl_select_state(himax_pinctrl, state);
	if (error)
