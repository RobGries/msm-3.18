				...
				u32 brightness)
{
	struct sn3193_led_pdata *pdata = data;
	bool on_off = power;
	printk("%s, on=%d, led_current=%d, led_brightness=%d color %d\n",
					...
					bool power, enum led_colors color)
{
	struct sn3193_led_pdata *pdata = data;
	bool on_off = power;
	printk("%s, on=%d, t0 %d t1 %d t2 %d t3 %d t4 %d\n",
				  ...
				  const char *buf, size_t count)
{
	...
	if (blinking > 0)
		...
		hold_time = 0;
//	printk("%s led_cdev->name %s blinking %ld ret %d\n", __func__,
...
static DEVICE_ATTR(blink, 0644, NULL, sn3193_blink_store);
...
MODULE_AUTHOR("Ren Hongwei <renhw0422@thundersoft.com>");
MODULE_DESCRIPTION("SI-EN Breath LED Driver");
MODULE_LICENSE("GPL");
